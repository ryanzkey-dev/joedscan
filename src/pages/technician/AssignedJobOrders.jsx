...
export default function AssignedJobOrders() {
  ...
  const load = async () => {
    ...
    setError('')
    try {
      const res = await apiRequest('getTechnicianJobOrders', { technicianId: user.id })
  ...
  const handleStatusChange = async (id, status) => {
    setError('')
    try {
      await apiRequest('updateJobOrderStatus', { jobOrderId: id, status })
      await load()
    } catch (err) {
      setError(err.message)
    }
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-xl font-bold text-gray-800">Assigned Job Orders</h1>
